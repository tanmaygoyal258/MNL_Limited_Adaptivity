...
from utils import sigmoid , dsigmoid , probit , dprobit


def fit_online_logistic_estimate(arm, reward, current_estimate, vtilde_matrix, vtilde_inv_matrix, constraint_set_radius,
                                diameter=1, precision=0.1 , model = "logistic"):
    """
    ECOLog estimation procedure.
    """

    reward_func = sigmoid if model == "logistic" else probit
    d_reward_func = dsigmoid if model == "logistic" else dprobit 
    ...
    for _ in range(iters):
        pred_probas = reward_func(np.sum(z_estimate * inv_z_arm))
        grad = z_estimate - z_theta_t + ((pred_probas - reward) / (pred_probas * (1-pred_probas))) * d_reward_func(np.sum(z_estimate * inv_z_arm))  * inv_z_arm
...
def fit_online_logistic_estimate_bar(arm, current_estimate, vtilde_matrix, vtilde_inv_matrix, constraint_set_radius,
    ...
    for _ in range(iters):
        pred_probas = reward_func(np.sum(z_estimate * inv_z_arm))
        grad = z_estimate - z_theta_t + ((2*pred_probas - 1) / (pred_probas * (1-pred_probas))) * d_reward_func(np.sum(z_estimate * inv_z_arm))  * inv_z_arm
        unprojected_update = z_estimate - step_size * grad
        z_estimate = project_ellipsoid(x_to_proj=unprojected_update,
                                    ell_center=np.zeros_like(arm),
                                    ecc_matrix=vtilde_matrix,
